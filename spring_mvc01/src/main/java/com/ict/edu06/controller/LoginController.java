...
public class LoginController {
	...
	public ModelAndView getLoginOK(LoginVO lvo) {
	    ...
	    try {
	        ...
	        if (loginVO != null) {
	            // 2. 사용자가 입력한 암호를 추출
	            String userPw = lvo.getM_pw();

	            // 3. 두 암호를 비교해서 같으면 성공, 다르면 실패
	            // passwordEncoder.matches(암호화 안된 것, 암호화된 것) => 일치하면 true, 아니면 false
	            if (passwordEncoder.matches(userPw, loginVO.getM_pw())) {
