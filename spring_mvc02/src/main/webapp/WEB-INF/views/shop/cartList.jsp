...
<body>
	...
	<table>
					...
					<c:forEach var="k" items="${cart_list}">
						<tr>
							...
							<td>시중가 : <fmt:formatNumber value="${k.p_price}"
									pattern="#,##0" />원<br> <font color="red">할인가 : <fmt:formatNumber
										value="${k.p_saleprice}" pattern="#,##0" />원
							</font></td>
							<td>
						...
						<c:set var="cartTotal" value="${cartTotal + (k.p_price * k.p_su)}" />
					</c:forEach>
				</c:otherwise>
			</c:choose>
		</tbody>
		<tfoot>
			<tr>
				<td colspan="6" style="text-align: right; padding-right: 40px;">총 결제액:${cartTotal}</td>
			</tr>
		</tfoot>
	</table>
